...
class MainActivity : AppCompatActivity() {
 ...
 private fun startRender(){createButton.isEnabled=false;status.text="Rendering MP4…";Thread{try{render()}catch(e:Exception){runOnUiThread{status.text="Render failed: ${e.message}";createButton.isEnabled=true}}}.start()}
 ...
 private fun render(){
  ...
  val mmr=MediaMetadataRetriever();mmr.setDataSource(this,audio);val durationUs=(mmr.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)?.toLongOrNull()?:error("Could not read duration"))*1000L;mmr.release()
  val bitmap=contentResolver.openInputStream(art).use{requireNotNull(BitmapFactory.decodeStream(it))}
  ...
  while(vt<0||at<0){if(vt<0){val x=venc.dequeueOutputBuffer(vi,10000);if(x==MediaCodec.INFO_OUTPUT_FORMAT_CHANGED)vt=mux.addTrack(venc.outputFormat);else if(x>=0)venc.releaseOutputBuffer(x,false)};if(at<0){val x=aenc.dequeueOutputBuffer(ai,10000);if(x==MediaCodec.INFO_OUTPUT_FORMAT_CHANGED)at=mux.addTrack(aenc.outputFormat);else if(x>=0)aenc.releaseOutputBuffer(x,false);if(at<0){val ix=aenc.dequeueInputBuffer(10000);if(ix>=0){aenc.queueInputBuffer(ix,0,0,0,0)}}}}
  mux.start();val paint=Paint(Paint.ANTI_ALIAS_FLAG or Paint.FILTER_BITMAP_FLAG);var sec=0L;val total=(durationUs+999999)/1000000
  while(sec<=total){val c=surface.lockCanvas(null);try{c.drawColor(Color.BLACK);val s=minOf(w.toFloat()/bitmap.width,h.toFloat()/bitmap.height);val dw=bitmap.width*s;val dh=bitmap.height*s;c.drawBitmap(bitmap,null,RectF((w-dw)/2,(h-dh)/2,(w+dw)/2,(h+dh)/2),paint)}finally{surface.unlockCanvasAndPost(c)};drainVideo(venc,mux,vt,vi,false);sec++}
  venc.signalEndOfInputStream();drainVideo(venc,mux,vt,vi,true)
  transcodeAudio(audio,aenc,mux,at)
  venc.stop();venc.release();surface.release();aenc.stop();aenc.release();mux.stop();mux.release();ofd.close();bitmap.recycle();runOnUiThread{status.text="Saved: Movies/Cover Art Video Maker/$name";createButton.isEnabled=true;Toast.makeText(this,"MP4 created",Toast.LENGTH_LONG).show()}
 }
 private fun drainVideo(enc:MediaCodec,mux:MediaMuxer,track:Int,info:MediaCodec.BufferInfo,end:Boolean){while(true){val x=enc.dequeueOutputBuffer(info,if(end)10000 else 0);if(x==MediaCodec.INFO_TRY_AGAIN_LATER){if(!end)return}else if(x>=0){val b=enc.getOutputBuffer(x);if(b!=null&&info.size>0){b.position(info.offset);b.limit(info.offset+info.size);mux.writeSampleData(track,b,info)};val eos=info.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM!=0;enc.releaseOutputBuffer(x,false);if(eos)return}}}
 private fun transcodeAudio(uri:Uri,encoder:MediaCodec,mux:MediaMuxer,track:Int){
  val ex=MediaExtractor();val fd=contentResolver.openFileDescriptor(uri,"r")?:error("Audio open failed");ex.setDataSource(fd.fileDescriptor);var idx=-1;for(i in 0 until ex.trackCount){if(ex.getTrackFormat(i).getString(MediaFormat.KEY_MIME)?.startsWith("audio/")==true){idx=i;break}};require(idx>=0);ex.selectTrack(idx);val inf=ex.getTrackFormat(idx);val mime=inf.getString(MediaFormat.KEY_MIME)?:error("Audio format missing");val dec=MediaCodec.createDecoderByType(mime);dec.configure(inf,null,null,0);dec.start();val di=MediaCodec.BufferInfo();val ei=MediaCodec.BufferInfo();var inputDone=false;var decoderDone=false
  while(!decoderDone){if(!inputDone){val ix=dec.dequeueInputBuffer(10000);if(ix>=0){val b=dec.getInputBuffer(ix)!!;val n=ex.readSampleData(b,0);if(n<0){dec.queueInputBuffer(ix,0,0,0,MediaCodec.BUFFER_FLAG_END_OF_STREAM);inputDone=true}else{dec.queueInputBuffer(ix,0,n,ex.sampleTime,0);ex.advance()}}};val ox=dec.dequeueOutputBuffer(di,10000);if(ox>=0){val pcm=dec.getOutputBuffer(ox);if(pcm!=null&&di.size>0){pcm.position(di.offset);pcm.limit(di.offset+di.size);var remaining=di.size;while(remaining>0){val ix=encoder.dequeueInputBuffer(10000);if(ix>=0){val ib=encoder.getInputBuffer(ix)!!;ib.clear();val n=minOf(remaining,ib.remaining());val old=pcm.limit();pcm.limit(pcm.position()+n);ib.put(pcm);pcm.limit(old);encoder.queueInputBuffer(ix,0,n,di.presentationTimeUs,0);remaining-=n;drainAudio(encoder,mux,track,ei,false)}}};decoderDone=di.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM!=0;dec.releaseOutputBuffer(ox,false)}};val ix=encoder.dequeueInputBuffer(10000);if(ix>=0)encoder.queueInputBuffer(ix,0,0,0,MediaCodec.BUFFER_FLAG_END_OF_STREAM);drainAudio(encoder,mux,track,ei,true);dec.stop();dec.release();ex.release();fd.close()
 }
 private fun drainAudio(enc:MediaCodec,mux:MediaMuxer,track:Int,info:MediaCodec.BufferInfo,end:Boolean){while(true){val x=enc.dequeueOutputBuffer(info,if(end)10000 else 0);if(x==MediaCodec.INFO_TRY_AGAIN_LATER){if(!end)return}else if(x>=0){val b=enc.getOutputBuffer(x);if(b!=null&&info.size>0){b.position(info.offset);b.limit(info.offset+info.size);mux.writeSampleData(track,b,info)};val eos=info.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM!=0;enc.releaseOutputBuffer(x,false);if(eos)return}}}
}
